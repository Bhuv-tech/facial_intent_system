...
export default function CandidateInterface({ user, onLogout }) {
  ...
  const submitFeedback = (fair) => {
    console.log("Candidate fairness feedback:", fair)
  ...
  if (view === 'consent') {
    return (
      <div className="flex-center" style={{ padding: '2rem' }}>
        <div className="glass-panel" style={{ maxWidth: '600px', padding: '3rem', animation: 'fadeIn 0.5s ease-out' }}>
          <h2 style={{ marginBottom: '1.5rem' }}>Interview Consent & Privacy</h2>
          <div style={{ textAlign: 'left', color: 'var(--text-secondary)', marginBottom: '2.5rem', lineHeight: '1.7' }}>
            <p>To ensure a professional and fair interview experience, this system uses real-time AI to analyze engagement signals for the recruiter.</p>
            <ul style={{ marginTop: '1.5rem', marginLeft: '1.2rem', display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
              <li><strong>Zero Bias:</strong> AI assist is for secondary insights, not primary decisions.</li>
              <li><strong>Absolute Privacy:</strong> No video or audio is ever recorded or saved.</li>
              <li><strong>Real-time Only:</strong> Analysis happens live and data is discarded immediately after.</li>
            </ul>
          </div>
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
            <button className="btn-secondary" onClick={onLogout}>Decline</button>
            <button className="btn-primary" onClick={() => setView('session')}>Accept & Continue</button>
          </div>
        </div>
      </div>
    )
      ...
      <div className="flex-center" style={{ padding: '2rem' }}>
        <div className="glass-panel" style={{ maxWidth: '500px', padding: '3rem', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginBottom: '2.5rem' }}>
            Thank you for participating. Your session data has been purged.
          </p>
          {feedbackSuccess ? (
            <div className="badge-green" style={{ padding: '1rem', borderRadius: '8px' }}>Thank you for your feedback! Logging out...</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <button className="btn-primary" onClick={() => submitFeedback(true)}>It was fair and professional</button>
              <button className="btn-secondary" onClick={() => submitFeedback(false)}>I have concerns about the process</button>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="app-container animate-in" style={{ display: 'flex', flexDirection: 'column', height: '100vh', padding: '1rem' }}>
      ...
      <div className="glass-panel" style={{ padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', marginBottom: '1rem', alignItems: 'center' }}>
        <div>
           <h2 style={{ margin: 0, fontSize: '1.1rem' }} className="accent-text">Live Interview Session</h2>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
           <span style={{ color: 'var(--success)', fontSize: '0.8rem' }}>● Secure AI Assist Active</span>
           <button className="btn-primary" style={{ background: 'var(--danger)', padding: '0.6rem 1.2rem' }} onClick={() => setView('feedback')}>Finish Interview</button>
        </div>
      </div>

      {/* Main View */}
      <div style={{ flex: 1, display: 'flex', gap: '1.5rem', padding: '1rem' }}>
        <div className="glass-panel" style={{ flex: 1, background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          {!remoteVideoRef.current?.srcObject && (
            <div style={{ position: 'absolute', color: 'var(--text-secondary)', textAlign: 'center' }}>
               <p style={{ fontSize: '1.2rem' }}>Interviewer's Live Video Feed</p>
          ...
          <div style={{ position: 'absolute', bottom: '30px', right: '30px', width: '280px', borderRadius: '12px', overflow: 'hidden', border: '2px solid var(--accent-primary)', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
            ...
            <div style={{ position: 'absolute', bottom: 10, left: 10, display: 'flex', gap: '8px', alignItems: 'center' }}>
               <div style={{ background: 'rgba(0,0,0,0.6)', padding: '2px 8px', borderRadius: '4px', fontSize: '0.7rem' }}>You (Candidate)</div>
